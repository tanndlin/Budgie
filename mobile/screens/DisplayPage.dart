...
class _DisplayPageState extends State<DisplayPage> {
  ...
  final categoryEdit = TextEditingController();
  ...
  Future<void> deleteCat(int index)
  async {
    id = global.userId;
    String? extraId = getAllCategories[index].id;
    print("Remove");
    print(getAllCategories[index].name);
    var response = await BaseClient().deleteCategory(id, extraId).catchError((err) {print("Fail");});
    if (response == null) {
      _showToast("Could not remove", true);
      print("response null");
    }
  ...
  Widget build(BuildContext context) {
    ...
    void showCatEditDialog(int index, BuildContext context)
    {
      setState(() {
        categoryEdit.text = getAllCategories[index].name;
      });
                        ...
                        child: TextButton(
                          ...
                          onPressed: () async {
                            ...
                            if (categoryEdit.text == "") {
                              ...
                              var cat = MyCategory(
                                id: getAllCategories[index].id,
                                userId: id,
                                name: categoryEdit.text,
                              );
                              print(myCategoryToJsonEdit(cat));
                              var response = await BaseClient().editCategory(cat).catchError((err) {print("Fail");});
                              if (response == null) {
                                _showToast("Could not edit", true);
                                print("response null");
                              }
    ...
    void showCatDeleteDialog(int index, BuildContext context)
    {
      String deleteName = getAllCategories[index].name;
    ...
    Widget _buildCatCard(int index) => Container(
      padding:  EdgeInsets.only(top: 2.0, left: 10.0, right: 10.0, bottom: 5.0),
      width: MediaQuery.of(context).size.width,
      height: 80,
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Row(
            children: <Widget>[
              Text(
                "${getAllCategories[index].name}", style: TextStyle(fontSize: 18, color: Colors.blueAccent.shade700, fontWeight: FontWeight.bold, decoration: TextDecoration.underline),
              ),
              Spacer(),
              // TextButton(onPressed: (){}, child: Icon(Icons.edit, color: Color(0xFF2D4B03))),
              IconButton(onPressed: () { showCatEditDialog(index, context);}, icon: Icon(Icons.edit, color: Color(0xFF2D4B03), size: 25,), padding: EdgeInsets.zero, constraints: BoxConstraints(),),
              SizedBox(width: 15.0,),
              IconButton(onPressed: () { showCatDeleteDialog(index, context);}, icon: Icon(Icons.delete, color: Color(0xFF2D4B03), size: 25,), padding: EdgeInsets.zero, constraints: BoxConstraints(),),
              // Container(
              //   // padding: EdgeInsets.symmetric(horizontal: 5.0),
              //   child: Row(
              //     // crossAxisAlignment: CrossAxisAlignment.end,
