...
function Divider(props) {
    ...
    return (
        <div id='divider' className={"duration-300 w-1/2 min-h-[348px] absolute right-0 group transform h-full bg-gradient-to-br from-[#479CE0] via-[#9ECFA2] to-[#E7E233]" + (props.dividerToggle ? '' : dividerPositionClass)} onClick={() => { props.setDividerToggle(!props.dividerToggle) }}>            
            <img className='absolute rounded-md h-full w-full opacity-[0.7]' src={DividerImage} alt='Wood Texture' />
            <div className='relative mx-32 mb-5'>
                <div className='absolute inset-x-0 top-5'>
                    <p id='dividerText' className={props.dividerToggle ? textVisible : textInvisible}>Sign Up</p>
                </div>
                <div className='absolute inset-x-0 top-5'>
                    <p id='dividerText' className={!props.dividerToggle ? textVisible : textInvisible}>Log In</p>
                </div>  
            </div>
